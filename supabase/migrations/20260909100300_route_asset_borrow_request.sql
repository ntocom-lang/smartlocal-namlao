-- ส่งคำขอ "ขอยืมพัสดุ/ครุภัณฑ์" (asset_borrow_request) เข้ากองที่ดูแล
--
-- ⚠️ ตามปกติ CASE ด้านล่างจะ **ไม่ทำงาน** กับประเภทนี้ — create_asset_borrow_request()
-- ระบุ department_id มาเองเป็น "กองเจ้าของพัสดุ" (กองที่ต้องจ่ายของและรับคืนจริง) ซึ่งชนะ
-- CASE เสมอตามเงื่อนไข IF NEW.department_id IS NULL
-- branch นี้เป็นตาข่ายรองรับกรณีเดียว: ของในทะเบียนยังไม่ได้ผูกกอง (department_id เป็น NULL)
-- ให้ตกไปที่งานพัสดุซึ่งตามผังกองอยู่ที่กองคลัง ถ้า อปท. ไม่มีกองคลังจึงไปสำนักปลัด
--
-- ⚠️ CREATE OR REPLACE เขียนทับทั้งฟังก์ชัน เนื้อด้านล่างจึงคัดมาครบจาก
-- 20260908200000_route_public_assistance_request.sql (ผังงานจาก document_type_assignments
