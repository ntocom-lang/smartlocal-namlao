-- ค่าตั้งต้นของช่อง "ผู้มีอำนาจสั่งใช้รถ" บนแบบ 3 ย้ายจาก "บทบาทตายตัว" มาเป็น
-- "เครื่องหมายที่แอดมินติ๊กเองที่แถวไหนก็ได้"
--
-- เดิมค่าตั้งต้นล็อกอยู่ที่บทบาท vehicle_authority แถวเดียว พอเปิดให้แอดมินสร้างแถวเองได้
-- แถวสำเร็จรูปนั้นก็ซ้ำซ้อนและค้างอยู่ในรายการทั้งที่ไม่ได้ใช้ ต่อไปติ๊กที่ปลัด หรือที่แถว
-- รองนายกที่สร้างเองก็ได้ — ยืดหยุ่นกว่าและหน้าจอสะอาดกว่า
--
-- ⚠️ ไฟล์นี้ "เพิ่มอย่างเดียว" โดยตั้งใจ ยังไม่ถอด vehicle_authority ออกจาก CHECK และยังไม่
-- revoke v3 เพราะ migration ถูก apply ก่อน deploy เสมอ ถ้าถอดตั้งแต่ตอนนี้ โค้ดที่รันอยู่บน
-- production จะบันทึกแถวนั้นไม่ได้และได้ error ที่อ่านไม่ออก (บทเรียนจาก 20260904160000)
-- ส่วนที่ถอดอยู่ในไฟล์ถัดไปซึ่ง apply หลัง deploy

-- ขยะจากการทดสอบบนสนามซ้อมของรอบพัฒนา ลบก่อนเปลี่ยน CHECK เพื่อไม่ให้แถวเก่าค้าง
-- ทำให้ ALTER TABLE ล้มภายหลัง (CHECK มีผลกับทุกแถว ไม่เว้นแถวที่ปิดไปแล้ว)
DELETE FROM public.document_signatories
WHERE is_active = false
  AND manual_name LIKE '[TEST]%';

ALTER TABLE public.document_signatories
  ADD COLUMN IF NOT EXISTS is_vehicle_order_default boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.document_signatories.is_vehicle_order_default IS
  'แถวนี้เป็นค่าตั้งต้นของช่องผู้มีอำนาจสั่งใช้รถบนใบขออนุญาตใช้รถ (แบบ 3) — ติ๊กได้แถวเดียวต่อ อปท.';

-- หัวหน้ากองไม่ใช่ผู้มีอำนาจสั่งใช้รถ กันไว้ทั้งที่ RPC และที่ตารางเผื่อมีทางเขียนอื่น
ALTER TABLE public.document_signatories
  DROP CONSTRAINT IF EXISTS document_signatories_vehicle_default_scope_check;
ALTER TABLE public.document_signatories
  ADD CONSTRAINT document_signatories_vehicle_default_scope_check
  CHECK (NOT is_vehicle_order_default OR signatory_role <> 'department_head')
  NOT VALID;

-- ติ๊กได้แถวเดียวต่อ อปท. ถ้ามีสองแถวระบบจะเลือกไม่ถูกว่าใครเป็นค่าตั้งต้น
CREATE UNIQUE INDEX IF NOT EXISTS document_signatories_one_vehicle_default_idx
  ON public.document_signatories (municipality_id)
  WHERE is_vehicle_order_default AND is_active;

-- v4 = v3 + เครื่องหมายค่าตั้งต้น ทำเป็นเวอร์ชันใหม่แทนการเติมพารามิเตอร์ให้ v3
-- ด้วยเหตุผลเดิม (PostgREST เลือก overload จากชุดชื่อ argument) และเพื่อให้ v3 ยังใช้ได้
-- ระหว่างรอ deploy โค้ดใหม่
...
REVOKE ALL ON FUNCTION public.set_document_signatory_v4(
  uuid, text, uuid, uuid, text, text, text, date, date, text, boolean
) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.set_document_signatory_v4(
  uuid, text, uuid, uuid, text, text, text, date, date, text, boolean
) TO authenticated;
