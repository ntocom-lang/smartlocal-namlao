-- role_sort ใน get_users_with_email_sorted() hardcode คำว่า 'สภาเทศบาล' ไว้ ทั้งที่หน่วยงานที่เป็น
-- อบต./อบจ. ต้องเป็น "สภา อบต." / "สภา อบจ." — ฟิลด์นี้ใช้เรียงลำดับคอลัมน์ "บทบาท" เท่านั้น
-- ไม่ได้ถูก SELECT ออกไปให้ client แสดงผล จึงเปลี่ยนเป็นคำกลาง 'สมาชิกสภา' ที่ถูกต้องกับทุกประเภท
-- หน่วยงาน แทนการ join municipalities มาแปลคำในระดับ DB (ซึ่งไม่ให้ประโยชน์อะไรเพิ่ม เพราะป้ายที่
-- ผู้ใช้เห็นจริงมาจาก ROLE_LABELS ฝั่ง client — ดู src/lib/orgTerms.js)
--
-- ลำดับการเรียงไม่เปลี่ยน: ทั้งสองคำขึ้นต้นด้วย 'ส' และ role นี้เป็นสมาชิกเดียวในกลุ่มนั้น
--
-- ตัวฟังก์ชันคัดลอกมาจาก 20260829124851_sort_user_management_columns.sql ทั้งดุ้น (ตรวจแล้วว่า
-- prosrc ใน production ตรงกับไฟล์นั้นทุกไบต์ ไม่มี drift) เปลี่ยนแค่บรรทัดเดียวคือ WHEN 'council'
-- CREATE OR REPLACE คง ACL เดิมไว้ แต่ยังใส่ REVOKE/GRANT ซ้ำท้ายไฟล์ให้ตรวจสอบได้ในตัว
