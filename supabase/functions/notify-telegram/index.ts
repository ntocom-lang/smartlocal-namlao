...
// จุดสีหน้าเส้นคั่นบรรทัดแรก (ดู framedMessage()) แยกตาม "กองที่รับผิดชอบ" ให้เจ้าหน้าที่กวาดตาหางานของกองตัวเอง
...
// ⚠️ พรีวิว push notification ยุบขึ้นบรรทัดเป็นช่องว่าง เส้นคั่นจึงกินที่ต้นพรีวิวไป 17 ตัวอักษร
...
// เจ้าของระบบขอเหลือเส้นบนเส้นเดียว ลบเส้นล่างออก (2569-09-13) — ข้อความใบถัดไปเริ่มด้วยเส้นของตัวเอง
// จึงยังแยกใบได้ ไม่ต้องใส่เส้นปิดท้ายกลับเข้ามา
const RULE_DASH = '┈'
...
// ประกอบข้อความทั้งใบ: เส้นคั่นสีกอง / <อีโมจิ> <หัวเรื่อง> / เนื้อหา
...
function framedMessage(resource: Record<string, unknown>, emoji: string, title: string, body: string[]) {
  ...
  return [rule, `${emoji} <b>${title}</b>`, ...body.filter(Boolean)].join('\n')
}
...
// รูปแบบของทุกใบ (ดู framedMessage()): เส้นคั่นสีกอง / <อีโมจิประเภทเรื่อง> <หัวเรื่อง> / เนื้อหา
