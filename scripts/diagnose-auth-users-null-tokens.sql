-- ✅ แก้ไปแล้วเมื่อ 2026-08-30 — เก็บไฟล์นี้ไว้เผื่ออาการกลับมา ไม่ใช่ปัญหาที่ค้างอยู่
--
-- สรุปเคสจริง: auth.admin.listUsers() ตอบ "Database error finding users" อยู่พักหนึ่ง
-- ตัวการคือแถวบัญชีทดสอบ test-*@smartlocal.test (uuid ตายตัว 00000000-...) ที่ seed ชุดเก่า
-- INSERT ลง auth.users ตรงๆ โดยไม่ได้ตั้งค่าช่อง token ปล่อยเป็น NULL ไว้ — พอลบแถวพวกนั้น
-- ทิ้ง (ตอนกวาด auth user ที่ไม่มี profile) listUsers ก็กลับมาทำงานปกติทันที ยืนยันด้วยการ
-- ยิงจริง: อ่านครบ 95 บัญชี และไล่ทีละแถวด้วย perPage=1 ผ่านหมดไม่มีแถวไหนพัง
--
-- seed ชุดปัจจุบัน (supabase/seed_demo_accounts.sql) ตั้งช่อง token เป็น '' ทุกช่องอยู่แล้ว
-- อาการนี้จึงไม่ควรเกิดซ้ำจากการ seed อีก
--
-- ── ด้านล่างคือคิวรีตรวจเดิม ใช้เมื่ออาการกลับมา ────────────────────────────────
