...
export async function hydroHourlyResponse(request, { fetcher = fetch, cache = globalThis.caches?.default, now = Date.now() } = {}) {
  ...
  try {
    ...
    const response = await fetcher(`https://hydro1.ddns.net/main/information_6/water_today_search_json.php?${params}`, { signal: AbortSignal.timeout(10000), redirect: 'manual' })
    ...
    console.warn('Hydro upstream unavailable', error.message)
