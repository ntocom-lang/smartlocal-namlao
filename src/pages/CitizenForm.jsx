...
export default function CitizenForm() {
  ...
  // ช่องรายละเอียด: ทุกหมวดต้องกรอกเสมอ แต่ไม่จำกัดความยาวขั้นต่ำแล้ว (เจ้าของระบบสั่ง 2569-09-17)
  // ของเดิมบังคับ 10 ตัวอักษรทุกหมวดยกเว้นกลิ่นเหม็นรบกวน ผู้ร้องที่พิมพ์สั้นแต่สื่อความชัด
  // เช่น "ไฟดับ" จึงถูกปฏิเสธ
  //
  // ⚠️ กฎเดียวกันนี้บังคับซ้ำที่ฐานข้อมูล (submit_citizen_complaint_v4 ในไมเกรชัน
  // 20260917110000) ที่นี่เป็นแค่การแจ้งเตือนให้เห็นเร็ว แก้ที่เดียวไม่พอ ต้องแก้ทั้งสองที่
  function validateDetail(form) {
    if (!form.detail.trim()) return 'กรุณากรอกรายละเอียด'
    return null
  }
      ...
      <form onSubmit={handleSubmit} className="px-3 pt-3 pb-24 space-y-2">
        ...
        {form.category === 'odor' && (
          <div className="space-y-2">
            ...
            <div className="rounded-xl border border-lime-200 bg-lime-50/80 px-3.5 py-2.5 text-[11px] leading-relaxed text-lime-900">
              <p className="font-bold mb-1 flex items-center gap-1.5 text-xs text-lime-950">
                🔒 คำร้องเฉพาะกิจ · ข้อมูลของท่านเป็นความลับ
              </p>
              <p>
                การแจ้งเรื่องในหมวดเฉพาะกิจนี้จะถูกเก็บรักษาเป็นความลับ ไม่เปิดเผยข้อมูลต่อสาธารณะ
                ข้อมูลคำร้อง พิกัด และข้อมูลติดต่อของท่านจะถูกส่งตรงถึงเจ้าหน้าที่ผู้รับผิดชอบโดยเฉพาะ
                เพื่อใช้ในการลงพื้นที่ตรวจสอบข้อเท็จจริงและติดต่อแจ้งผลการดำเนินงานกลับ
              </p>
              <p className="mt-1 text-lime-800">
                โดย{tenant?.name ?? 'หน่วยงาน'}คุ้มครองข้อมูลส่วนบุคคลของท่าน และจำกัดการเข้าถึงเฉพาะเจ้าหน้าที่ผู้รับผิดชอบเรื่องและผู้ดูแลระบบเท่านั้น
              </p>
            </div>
        ...
        {/* Detail */}
        {/* ถอด minLength ออกแล้ว (ไม่จำกัดความยาวขั้นต่ำ 2569-09-17) — required ยังอยู่ ห้ามปล่อยว่าง
            ⚠️ minLength ของเบราว์เซอร์บล็อกตั้งแต่ก่อนโค้ดเราได้ทำงาน ถ้าใส่กลับมาต้องแก้ทั้ง
            validateDetail() และฐานข้อมูลให้ตรงกันทั้งสามที่ */}
