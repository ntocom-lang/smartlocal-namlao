...
export default function PatientTransportBooking() {
  ...
  // ⚠️ "รับจองแทน" ต้องมาจากการกดปุ่มในหน้าจัดคิวเท่านั้น ห้ามอนุมานจากบทบาท
  // เจ้าหน้าที่จัดคิวที่จองให้ตัวเอง/ญาติต้องอยู่ใต้กติกาเดียวกับประชาชน (ฐานข้อมูลตรวจซ้ำอีกชั้น)
  ...
  return <div className="mx-auto min-h-screen max-w-5xl bg-white px-4 py-6 text-slate-900">
    ...
    {current && <>
      ...
      {view === 'calendar' && info?.enabled && <BookingCalendar tenantId={tenantId} info={info} uid={uid} onBook={seed => { setBookingSeed(seed); setStaffEntry(false); setView('book') }} />}
      {view === 'book' && uid && info?.enabled && <BookingForm submitError={error} tenantId={tenantId} initial={bookingSeed} info={info} profileName={profileName} profilePhone={workspace?.my_profile?.phone} staffEntry={staffEntry && isCoordinator} busy={busy} onBack={() => setView('home')} onSubmit={(id, payload) => mutate(bookingSeed.requested_trip_id ? 'patient_booking_submit_join' : 'patient_booking_submit', { p_id: id, p_data: payload, p_staff_entry: staffEntry && isCoordinator, ...(bookingSeed.requested_trip_id ? { p_trip: bookingSeed.requested_trip_id } : {}) }, 'รับคำขอแล้ว รอเจ้าหน้าที่ยืนยันรถ', () => setView('mine'))} />}
      ...
      {view === 'queue' && isCoordinator && <><button className={`${buttonClass} mb-4`} disabled={busy || !info?.enabled} onClick={() => { setBookingSeed({}); setStaffEntry(true); setView('book') }}>รับจองแทนทางโทรศัพท์/หน้าเคาน์เตอร์</button>
