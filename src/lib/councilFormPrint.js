...
function buildRequestCopy(category, categoryLabel, tenantName) {
  ...
  // tree แยกออกจาก canal เพราะหัวเรื่องต้องไม่ผูกกับ label ของหมวด — label มาจากแถวใน
  // complaint_categories ที่แต่ละ อปท. ตั้งเอง ส่วนใหญ่ตั้งเป็นคำกริยา ("ตัดต้นไม้") พอต่อท้าย
  // "เกี่ยวกับ" จะได้ "…ดำเนินการเกี่ยวกับตัดต้นไม้" ซึ่งผิดรูปหนังสือราชการ ต่อให้ไปแก้ค่า
  // fallback ในโค้ดก็ไม่ช่วย เพราะ label จาก DB ทับ fallback เสมอ จึงต้องตัดที่ template นี้แทน
  if (category === 'tree') {
    return {
      subject: 'ขอความอนุเคราะห์ตัดแต่งกิ่งไม้/ต้นไม้',
      body: `มีความประสงค์ขอความอนุเคราะห์${org}ตรวจสอบและดำเนินการตัดแต่งกิ่งไม้หรือต้นไม้ที่อาจก่อให้เกิดอันตราย ณ สถานที่และตามรายละเอียดที่ระบุด้านล่าง`,
    }
  }
