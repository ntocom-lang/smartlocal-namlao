...
${senderAddress.map(part => `      <p>${esc(part)}</p>`).join('\n')}
  ...
  <p class="kv kv--wide"><span class="bold">สิ่งที่ส่งมาด้วย</span>&nbsp;&nbsp;ใบคำขอรับสวัสดิการ (รถรับ-ส่งผู้ป่วย)&nbsp;&nbsp;จำนวน ${attachmentCount} ฉบับ</p>
...
function formSheet({
  ...
  <p class="form-para">${field('ข้าพเจ้า', requesterName, '65mm')} ${
    ...
    form.beneficiary_of_name
      ...
      : ''
  } มีความประสงค์ขอความอนุเคราะห์รถรับ-ส่งผู้ป่วยจากกองทุน รายละเอียดตามตารางท้ายนี้</p>
...
${rows.map(([label, value]) => `    <tr><th>${label}</th><td>${value ? esc(value) : '&nbsp;'}</td></tr>`).join('\n')}
  ...
  <div class="sign-block center-row" style="margin-top:3mm">
    ...
    ${form?.signed_by?.channel === 'booking'
      ...
      // ⚠️ ห้ามเปลี่ยนเป็นข้อความของโหมด online — ผู้ยื่นไม่ได้ยืนยันตัวตนในระบบ การเขียนว่า
      // ยืนยันแล้วคือการอ้างข้อเท็จจริงเท็จบนเอกสารที่ส่งให้องค์กรภายนอกใช้ประกอบการอนุมัติ
      // ไม่ใส่ชื่อ อปท. ในบรรทัดนี้ — บรรทัดกำกับที่มาท้ายใบ (.origin) บอกอยู่แล้ว ใส่ซ้ำทำให้
      // ข้อความติดกันเป็นพืด ("เจ้าหน้าที่องค์การบริหารส่วนตำบลทุ่งแค้วบันทึก…") และยาวเกิน 1 บรรทัด
      : `<p class="signed-note">เจ้าหน้าที่บันทึกคำขอแทนที่เคาน์เตอร์${
          counterStamp ? ` เมื่อ ${esc(counterStamp)}` : ''}${referenceNo ? ` · เลขอ้างอิง ${esc(referenceNo)}` : ''}
  ...
  <p class="note-template">
    ...
    ปรับให้ใช้เฉพาะเรื่องรถรับ-ส่งผู้ป่วย · หากกองทุนมีแบบของตนเองให้ใช้แบบนั้นแทน
