...
// ใน DOC_TITLES/buildDocBody (StaffDashboard.jsx) และตัวแปลชื่อของ MyDocRequests/LpaDocStats
// เพราะคำขอเก่าที่เคยยื่นไว้ก่อนตัดออกยังอยู่ในระบบและต้องแสดงผล/พิมพ์เอกสารได้ตามปกติ
 ...
 * ใบอนุญาตก่อสร้างบ้านทิ้ง อีกที่หนึ่งยังต้องใช้ ค่านี้จึงเป็นรายชื่อ "ไม่ใช้ที่นี่" รายหน่วยงาน
 *
 ...
 * จะโชว์เป็น 'building_permit' ดิบๆ และรายงาน LPA จะนับไม่ตรง กรองเฉพาะจุดที่เป็น
...
export const DEFAULT_SLA_DAYS = {
  ...
  // เตือนเจ้าหน้าที่ว่าเรื่องค้าง ไม่ใช่คำมั่นว่าจะช่วยเหลือเสร็จภายในกี่วัน
  public_assistance_request: 5,
  // ต้องตรวจว่าของว่างจริงในช่วงวันที่ขอ เสนอความเห็นปลัดฯ แล้วให้นายกฯ อนุมัติ — สามขั้น
  // แต่ต้องเร็วกว่าใบอื่น เพราะผู้ยืมมักขอล่วงหน้าไม่กี่วันก่อนถึงวันงาน อนุมัติช้าก็ไม่ทันใช้
