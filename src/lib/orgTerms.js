...
// ตำแหน่งปลัดพร้อมชื่อหน่วยงานเต็ม — คู่กับ orgHeadTitle() ใช้เป็นค่าสำรองของช่องลงนามในเอกสาร
// ที่พิมพ์ออกกระดาษ เมื่อทะเบียนผู้ลงนามยังไม่มีแถวของปลัดคนปัจจุบัน
//
// ต่างจาก orgHeadTitle ตรงที่ไม่ต้องตัดคำนำหน้าชื่อหน่วยงานออก เพราะตำแหน่งคือ "ปลัด" + ชื่อเต็ม
// ตรงๆ ทั้งสองแบบ (ปลัดเทศบาลตำบลน้ำเลา / ปลัดองค์การบริหารส่วนตำบลทุ่งแค้ว) ไม่ใช่
// "ปลัด อบต.ทุ่งแค้ว" ซึ่งเป็นคำเรียกแบบย่อที่ใช้ในหน้าจอระบบ
export function orgClerkTitle(tenant) {
  const name = (typeof tenant === 'string' ? tenant : tenant?.name)?.trim() || 'หน่วยงาน'
  return `ปลัด${name}`
}
