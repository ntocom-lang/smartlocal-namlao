...
const citizen = [
  ...
  ['home', 'ส่งคำขอจองรถ', 'ที่ “หน้าบริการ” กด “ขอจองรถรับส่ง” หรือเข้าสู่ระบบก่อน ระบุวันนัด โรงพยาบาล จุดรับ เบอร์ติดต่อ ผู้ติดตาม และการใช้รถเข็นหรือเปล เลือกรอรับกลับ กลับมารับภายหลัง หรือขาไปอย่างเดียว ตรวจข้อมูลและข้อความการใช้ข้อมูลก่อนส่ง'],
...
const coordinator = [
  ...
  ['queue', 'รับจองและตรวจคำขอ', 'เปิด “จัดคิว” รับจองแทนทางโทรศัพท์หรือหน้าเคาน์เตอร์ได้ ตรวจวันนัด จุดรับ ผู้ติดตาม รถเข็นหรือเปล และประสานข้อมูลที่ไม่ครบก่อนจัดเที่ยว'],
  ['queue', 'ตรวจแผนก่อนยืนยัน', 'กด “ตรวจแผนและเวลาว่าง” ระบบช่วยตรวจเวลาและความจุ เจ้าหน้าที่ตรวจผลและกดยืนยันเที่ยว หากข้อมูลเปลี่ยนให้ตรวจแผนใหม่ ผู้จองและคนขับจะเห็นผลในระบบ'],
  ...
  ['queue', 'แก้เหตุขัดข้องและข้อมูล', 'ประสานคนขับเมื่อมีเหตุขัดข้องหรือขอยกเลิก แก้ข้อมูลตามข้อเท็จจริงพร้อมเหตุผล หากระบบแจ้งว่าข้อมูลเปลี่ยนแล้ว ให้ตรวจค่าล่าสุดก่อนบันทึกใหม่'],
  ['queue', 'เอกสารและสรุปรายเดือน', 'ในเที่ยวที่ยืนยันแล้ว บันทึกเลขหนังสือและตรวจเลขไมล์ก่อนพิมพ์ หนังสือนำส่งเป็นร่างให้สารบรรณตรวจ สรุปรายเดือนเป็นแบบกลาง หากกองทุนมีแบบของตนให้ใช้แบบนั้น เลขไมล์ผิดปกติจะไม่ใช้คำนวณระยะทางจนตรวจแก้'],
]
const driver = [
...
const setup = [
  ['settings', 'ตั้งค่าก่อนเปิดบริการ', 'ผู้ดูแลเลือกกองทุน คนขับ เจ้าหน้าที่จัดคิว ความจุรถ เวลาให้บริการ เส้นทาง และตรวจปฏิทินวันหยุด ตั้งค่าตามบริการจริง แล้วตรวจข้อความการใช้ข้อมูลกับผู้รับผิดชอบก่อนเปิดจอง'],
  ...coordinator,
]
...
export default function BookingHelp({ enabled, coordinator: canCoordinate, driver: canDrive, admin, signedIn, busy, onHighlight }) {
  ...
  const steps = !enabled && role.id === 'citizen' ? [['home', 'บริการยังไม่เปิดจองคิวออนไลน์', 'เปิด “หน้าบริการ” แล้วเลือกยื่นคำขอรถรับ-ส่งผู้ป่วยผ่านช่องทางเดิม หรือติดต่อเจ้าหน้าที่ ติดตามคำขอที่ “เอกสารของฉัน” ตารางวันว่างและการร่วมเที่ยวจะใช้ได้เมื่อหน่วยงานเปิดระบบจอง']] : role.steps
