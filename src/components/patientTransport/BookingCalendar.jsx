...
import { buttonClass, primaryClass, inputClass, thaiDay, RETURN_MODES } from '../../lib/patientBooking'
...
function initialMode() { try { return localStorage.getItem('patient-calendar-view') === 'table' ? 'table' : 'calendar' } catch { return 'calendar' } }
...
export default function BookingCalendar({ tenantId, info, uid, onBook }) {
  ...
  function switchMode(value) { setMode(value); try { localStorage.setItem('patient-calendar-view', value) } catch { /* Storage may be disabled. */ } }
  ...
  return <section className="space-y-4" aria-label="ตารางรถสำหรับประชาชน">
    ...
    {data?.enabled && <>
      ...
      {mode === 'calendar' ? <>
        ...
        <table className="block w-full table-fixed sm:table"><caption className="sr-only">เที่ยวรถและช่วงว่างเรียงตามวัน</caption><thead className="hidden sm:table-header-group"><tr><th className="w-1/4 p-3 text-left">วันเดินทาง</th><th className="p-3 text-left">เที่ยวรถ / ช่วงว่าง / ขอจอง</th></tr></thead><tbody className="block sm:table-row-group">{visibleDays.map(d => <tr key={d.date} className="mb-3 block rounded-xl border border-slate-200 sm:table-row"><th scope="row" className="block p-3 text-left align-top sm:table-cell">{dateLabel(d.date)}<span className="block text-sm font-normal">{dayLabels[d.status]}</span></th><td className="block space-y-3 p-3 pt-0 sm:table-cell sm:pt-3">{freeDetails(d)}{d.trips.filter(matches).map(t => <article key={t.id} className="rounded-xl bg-slate-50 p-3">{tripDetails(t)}</article>)}{!d.free.length && !d.trips.filter(matches).length && <p>ไม่มีช่วงว่างหรือเที่ยวตรงกับตัวกรอง</p>}</td></tr>)}</tbody></table>
