...
export default function FloatingAIChat() {
  ...
  const [messages, setMessages] = useState([
    { id: 1, sender: 'bot', text: 'สวัสดีค่ะ ถามน้องสมายล์ได้เลยนะคะ 🦖' }
  ])
      ...
      <div className={`fixed z-[60] transition-all duration-300 ${
        ...
        <div className="w-[calc(100vw-24px)] max-w-sm bg-white rounded-3xl shadow-2xl border border-gray-200 overflow-hidden flex flex-col"
          ...
          <div className="flex items-center gap-3 px-4 py-3 text-white shrink-0"
            ...
            <div className="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center text-xl border border-white/30 shrink-0">🦖</div>
            <div className="flex-1 min-w-0">
              <p className="font-bold text-sm leading-tight">น้องสมายล์ AI</p>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-gradient-to-b from-blue-50/50 to-white">
            {messages.map(msg => (
              <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.sender === 'bot' && (
                  <div className="w-6 h-6 rounded-full bg-white flex items-center justify-center shadow-sm mr-1.5 shrink-0 border border-yellow-300 text-sm">🦖</div>
                )}
