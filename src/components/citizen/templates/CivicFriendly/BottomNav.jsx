...
export default function CivicFriendlyBottomNav() {
  ...
  return (
    <>
      <div
        className="md:hidden fixed bottom-0 left-0 right-0 z-50 flex items-stretch justify-around px-2 bg-white/90 backdrop-blur-md"
        style={{
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              ...
              className="flex-1 flex flex-col items-center gap-1 py-1 transition-transform active:scale-95 relative"
            >
              <div className="relative z-10">
                <Icon
                  size={24}
                  className={`transition-all ${isActive ? 'text-[var(--color-primary)]' : 'text-gray-300'}`}
      ...
      <div className="md:hidden" style={{ height: 'calc(3.5rem + max(env(safe-area-inset-bottom, 0px), 12px))' }} />
    </>
  )
}
