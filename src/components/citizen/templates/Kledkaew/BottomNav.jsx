...
export default function BottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden"
         style={{
           background: 'linear-gradient(180deg, #005ce6 0%, #004080 100%)',
      ...
      <div className="flex justify-around items-end h-[60px] px-2 relative z-10 pb-1.5">
        <Link to="/" className={`flex flex-col items-center gap-1 p-2 ${location.pathname === '/' ? 'text-white drop-shadow-md' : 'text-blue-200 hover:text-white'}`}>
          <Home size={22} strokeWidth={location.pathname === '/' ? 2.5 : 2} />
          <span className="text-[10px] font-bold tracking-wide">หน้าแรก</span>
        </Link>
        <Link to="/news" className={`flex flex-col items-center gap-1 p-2 ${location.pathname.startsWith('/news') ? 'text-white drop-shadow-md' : 'text-blue-200 hover:text-white'}`}>
        ...
        <Link to="/search" className="relative -top-5 flex flex-col items-center justify-center w-16 h-16 shadow-xl transition-transform active:scale-95"
              style={{
                ...
                boxShadow: '0 10px 25px -5px rgba(2, 132, 199, 0.6)'
              }}>
        ...
        <Link to="/near-me" className={`flex flex-col items-center gap-1 p-2 ${location.pathname === '/near-me' ? 'text-white drop-shadow-md' : 'text-blue-200 hover:text-white'}`}>
          <MapPin size={22} strokeWidth={location.pathname === '/near-me' ? 2.5 : 2} />
          <span className="text-[10px] font-bold tracking-wide">ใกล้ฉัน</span>
        </Link>
        <Link to="/more" className={`flex flex-col items-center gap-1 p-2 ${location.pathname === '/more' ? 'text-white drop-shadow-md' : 'text-blue-200 hover:text-white'}`}>
