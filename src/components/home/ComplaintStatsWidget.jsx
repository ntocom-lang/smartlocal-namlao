...
function ComplaintStatsWidget() {
  ...
  return (
    <Link to="/reports/complaints" aria-label="ดูสถิติเรื่องร้องเรียนร้องทุกข์"
      className="block rounded-2xl bg-white shadow-sm border border-gray-100 p-3 sm:p-5 hover:shadow-md active:scale-[0.99] transition-all">
      <p className="text-[15px] sm:text-base font-bold text-gray-800 mb-2.5 sm:mb-4">สถิติเรื่องร้องเรียนร้องทุกข์</p>
      ...
      <div className="flex items-start gap-2.5 sm:gap-6">
        <div className="shrink-0 flex flex-col items-center gap-1.5">
          <div className="relative h-[84px] w-[84px] sm:w-36 sm:h-36 rounded-full flex items-center justify-center"
            style={{ background: `conic-gradient(#1c5c96 ${resolvedPct}%, #6fb3d9 0)` }}>
            <div className="absolute inset-1.5 sm:inset-2.5 rounded-full bg-white flex flex-col items-center justify-center text-center px-1">
              <span className="text-[8px] sm:text-[11px] text-gray-400 leading-tight">จำนวนเรื่อง<br />ร้องเรียน</span>
              <span className="text-lg sm:text-3xl font-black text-gray-800 leading-none mt-0.5 sm:mt-1">{stats.total}</span>
            </div>
          </div>
        </div>
        <div className="flex-1 min-w-0">
          <div className="grid grid-cols-3 gap-1.5 sm:gap-2.5">
            {BOXES.map(b => (
              <div key={b.label} className="rounded-xl px-1 sm:px-2 py-1.5 sm:py-3 text-center text-white shadow-sm" style={{ background: b.gradient }}>
                <p className="text-base sm:text-2xl font-black leading-tight">{b.value}</p>
                <p className="text-[9px] sm:text-[10px] font-semibold leading-tight opacity-90 mt-0.5">{b.label}</p>
              </div>
            ))}
          </div>
          <div className="mt-2 flex items-center justify-center text-[11px] sm:text-sm font-bold py-2 sm:py-3 rounded-xl text-white"
