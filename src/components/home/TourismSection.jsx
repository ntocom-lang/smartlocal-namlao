  ...
  return (
    <section className="relative w-full rounded-3xl overflow-hidden shadow-2xl">
      ...
      <div className="relative z-10 pt-4 flex flex-col">
        ...
        <div className="px-5 mb-3">
          ...
          <h2 className="leading-none mb-0.5">
            <span className="text-[1.9rem] font-black drop-shadow-lg"
                  style={{ color: catData?.from ?? '#34d399', textShadow: `0 0 24px ${catData?.glow ?? '#34d39960'}` }}>
              มา
            </span>
          ...
          {CATS.map(({ key, label, Icon, from, to, glow }) => {
            ...
            return (
              <button key={key}
                      ...
                      className="shrink-0 flex flex-col items-center gap-1 transition-all duration-250 active:scale-90"
                      style={{ minWidth: 52 }}>
                <div className="relative flex items-center justify-center rounded-xl transition-all duration-300"
                  ...
                  {count > 0 && (
                    <span className="absolute -top-1 -right-1 min-w-[16px] h-[16px] px-0.5 flex items-center justify-center rounded-full text-[9px] font-bold text-white"
                          style={{ background: isActive ? 'rgba(255,255,255,0.3)' : `linear-gradient(135deg, ${from}, ${to})` }}>
                      {count}
                    </span>
                  )}
                </div>
                <span className={`text-[10px] font-bold drop-shadow transition-all duration-200 ${isActive ? 'text-white scale-105' : 'text-white/60'}`}>
                  {label}
                </span>
              </button>
            )
        ...
        {/* ── Places ── */}
        <div className="backdrop-blur-xl rounded-t-3xl px-4 pt-3 pb-4 border-t border-white/10"
            ...
            <div className="flex flex-col gap-2 mb-3">
              ...
              {hero && (
                <button onClick={() => navigate(`/tourism/${hero.id}`)}
                        className="relative w-full rounded-2xl overflow-hidden active:scale-[0.97] transition-transform group"
                        style={{ height: 148 }}>
                  {hero.image_url
                    ? <img src={hero.image_url} alt={hero.name}
                           className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    : <div className="absolute inset-0 flex items-center justify-center text-5xl"
                           style={{ background: `linear-gradient(135deg, ${catData?.from}aa, ${catData?.to})` }}>
                        {catData?.emoji ?? '🏙️'}
                      </div>}
                  ...
                  <div className="absolute inset-x-0 top-0 h-0.5 rounded-t-2xl"
                       style={{ background: `linear-gradient(90deg, ${catData?.from}, ${catData?.to})` }} />
                  <div className="absolute top-2.5 left-2.5 flex items-center gap-1 px-2 py-0.5 rounded-full backdrop-blur-md"
                       style={{ background: `linear-gradient(135deg, ${catData?.from}cc, ${catData?.to}cc)` }}>
                  ...
                  <div className="absolute bottom-0 inset-x-0 px-3 py-2.5">
                    <div className="flex items-center gap-1 mb-0.5">
                      ...
                      <p className="text-[10px] text-white/60 font-medium">{catData?.label}</p>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {subs.map((place, idx) => (
                    <button key={place.id} onClick={() => navigate(`/tourism/${place.id}`)}
                            className="relative rounded-2xl overflow-hidden active:scale-[0.96] transition-transform group"
                            style={{ height: 100 }}>
                      {place.image_url
                        ? <img src={place.image_url} alt={place.name}
                               className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        : <div className="absolute inset-0 flex items-center justify-center text-4xl"
                               style={{ background: `linear-gradient(135deg, ${catData?.from}88, ${catData?.to})` }}>
                            {catData?.emoji ?? '🏙️'}
                          </div>}
                      <div className="absolute inset-0 bg-linear-to-t from-black/80 via-black/20 to-transparent" />
                      <div className="absolute top-2 left-2 w-5 h-5 rounded-full flex items-center justify-center backdrop-blur-md"
                           style={{ backgroundColor: 'rgba(255,255,255,0.2)', border: '1px solid rgba(255,255,255,0.3)' }}>
                        <span className="text-[9px] font-black text-white">{idx + 2}</span>
                      </div>
                      <p className="absolute bottom-2 left-2.5 right-2.5 text-[12px] font-bold text-white drop-shadow-lg leading-tight line-clamp-2">
                        {place.name}
                      </p>
                    </button>
                  ))}
          ...
          <button
            ...
            style={catData
              ? { background: `linear-gradient(135deg, ${catData.from}, ${catData.to})`, color: '#fff', boxShadow: `0 4px 16px ${catData.glow}` }
