      ...
      <div className="relative z-10 px-4 pt-3 pb-4">
        <div className="flex items-center justify-between mb-3 gap-2">
          <div className="flex items-center gap-2">
            ...
            <p className={`${titleColor} font-extrabold text-[15px] md:text-base tracking-wide ${clean ? '' : 'drop-shadow-sm'}`}>{clean ? 'ร้องเรียน/ร้องทุกข์' : '🚨 ร้องเรียน/ร้องทุกข์'}</p>
          </div>
          {clean ? (
            ...
            return (
              <button key={cat.value}
                ...
                <div className={`${clean ? 'w-20 h-20' : 'w-13 h-13'} flex items-center justify-center shadow-md`}
                  ...
                  <CategoryIcon emoji={emoji} size={clean ? 44 : 32} style={tenant?.category_icon_style} />
                </div>
