...
// คอลัมน์แรกย่อจาก "ลำดับที่" เหลือ "ที่" ตั้งแต่ #57 — ต้องเช็คที่ markup ไม่ใช่ค้นคำเปล่าๆ
// เพราะ "ที่" เป็นชิ้นส่วนของหัวคอลัมน์อื่นในใบเดียวกันด้วย ("สถานที่ไป", "วันที่")
// ถ้าเช็คแค่ html.includes('ที่') เทสต์จะผ่านตลอดแม้คอลัมน์นี้หายไปทั้งคอลัมน์
assert.match(html, /class="c-seq"[^>]*>ที่</,
  'หัวคอลัมน์ลำดับต้องเป็น "ที่" ในช่อง c-seq')
