...
try {
  ...
  for (const [width, height] of [[320, 568], [390, 844], [480, 782]]) {
    ...
    await page.getByRole('button', { name: 'แชร์สถานการณ์ในพื้นที่', exact: true }).click()
    ...
    await page.evaluate(() => {
      window.sharedPm25 = null
      Object.defineProperty(navigator, 'canShare', { configurable: true, value: ({ files }) => files?.[0]?.type === 'image/png' })
      Object.defineProperty(navigator, 'share', { configurable: true, value: async payload => { window.sharedPm25 = { text: payload.text, type: payload.files[0].type, size: payload.files[0].size, active: navigator.userActivation.isActive } } })
    })
    await page.getByRole('button', { name: 'แชร์ภาพสรุป', exact: true }).click()
    const shared = await page.evaluate(() => window.sharedPm25)
    assert.equal(shared.active, true)
    assert.equal(shared.type, 'image/png')
    assert(shared.size > 1000)
    assert.match(shared.text, /【พื้นที่】\n/)
    assert.match(shared.text, /\n\n【ค่าฝุ่น】/)
    assert.match(shared.text, /https:\/\/demo.rk-networks.com\/pm25/)
    ...
    await page.getByRole('button', { name: 'แชร์ภาพสรุป', exact: true }).click()
    await page.getByText(/เครื่องนี้ไม่รองรับแชร์ภาพโดยตรง/).waitFor()
