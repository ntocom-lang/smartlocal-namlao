...
try {
  ...
  await check('Android has no install button before readiness; late event installs directly; dismissal hides it', async () => {
    ...
    await page.getByRole('button', {name:'วิธีติดตั้งแอป', exact:true}).waitFor()
  ...
  await check('OEM prompt failure: explicit Chrome intent, safe tenant URL, copy fallback and narrow layout', async () => {
    ...
    await dialog.waitFor({state:'detached'})
    assert.equal(await page.getByRole('button', {name:'วิธีติดตั้งแอป',exact:true}).evaluate(element => document.activeElement === element), true)
    await context.close()
  })
  await check('iOS guide and legacy media query API', async () => {
    ...
    await page.getByRole('dialog').getByText('หากมี "เปิดเป็นเว็บแอป" ให้เปิดไว้').waitFor()
    assert.equal(await page.evaluate(() => window.installCalls), 0)
    await page.screenshot({path: artifacts + 'ios-guide.png'})
    await context.close()
  })
  await check('visible Android fallback opens Chrome steps and upgrades to native install when ready', async () => {
    const {page,context} = await setup()
    await page.setViewportSize({width:320,height:568})
    await page.getByRole('button',{name:'วิธีติดตั้งแอป',exact:true}).click()
    const dialog = page.getByRole('dialog')
    await dialog.getByText('ติดตั้งผ่านเมนู Chrome ได้ตามนี้').waitFor()
    await dialog.getByText('ติดตั้งและสร้างทางลัด',{exact:true}).waitFor()
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth),true)
    await page.screenshot({path:artifacts+'android-standard-fallback.png'})
    await page.evaluate(()=>window.sendInstallEvent('accepted',true))
    await dialog.getByRole('button',{name:'ติดตั้งแอป',exact:true}).click()
    assert.equal(await page.evaluate(()=>window.installCalls),1)
    assert.equal(await page.evaluate(()=>window.hadUserActivation),true)
