...
try {
  ...
  await check('late event upgrades open guide; dismissal closes without repeated prompt', async () => {
    const {page, context} = await setup()
    await floating(page).click()
    const dialog = page.getByRole('dialog')
    await dialog.getByText('ยังเรียกหน้าติดตั้งจากปุ่มนี้ไม่ได้', {exact: false}).waitFor()
    assert.equal(await dialog.getByRole('button', {name:'เปิดใน Chrome'}).count(), 0)
    await page.screenshot({path: artifacts + 'android-waiting.png'})
    await page.evaluate(() => window.sendInstallEvent('dismissed'))
    await dialog.getByRole('button', {name:'ติดตั้งแอป', exact:true}).waitFor()
    ...
    assert.equal(await page.evaluate(() => window.installCalls), 1)
    await floating(page).click()
    assert.equal(await dialog.getByRole('button', {name:'ติดตั้งแอป', exact:true}).count(), 0)
    await context.close()
  })
  ...
  await check('OEM no prompt: explicit Chrome intent, safe tenant URL, copy fallback and narrow layout', async () => {
