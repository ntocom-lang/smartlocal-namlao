  ...
  for (const elem of osmData.elements || []) {
    if (elem.type === 'way' && elem.nodes && elem.nodes.length >= 2) {
      ...
      if (coordinates.length >= 2) {
        // STRICT FILTER: Keep road/waterway only if coordinates fall inside Nam Lao polygon boundary
...
function run() {
  ...
  console.log('2. Processing roads strictly inside ตำบลน้ำเลา...')
  const roadsJson = JSON.parse(fs.readFileSync('scratch/test_roads.json', 'utf-8'))
  const filteredRoads = osmToGeoJSON(roadsJson, boundaryRings)

  console.log(`✅ Strictly Inside Nam Lao Roads Count: ${filteredRoads.features.length} features!`)
  fs.writeFileSync('public/demo-gis/roads_namlao.geojson', JSON.stringify(filteredRoads, null, 2), 'utf-8')

  console.log('3. Processing waterways strictly inside ตำบลน้ำเลา...')
  const waterJson = JSON.parse(fs.readFileSync('scratch/test_waterways.json', 'utf-8'))
  const filteredWater = osmToGeoJSON(waterJson, boundaryRings)

  console.log(`✅ Strictly Inside Nam Lao Waterways Count: ${filteredWater.features.length} features!`)
  fs.writeFileSync('public/demo-gis/waterways_namlao.geojson', JSON.stringify(filteredWater, null, 2), 'utf-8')

  console.log('ALL STRICT NAM LAO GIS FILES PROCESSED SUCCESSFULLY!')
}
